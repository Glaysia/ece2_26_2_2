...
\hypersetup{pdftitle={LAB3-22 · 스텝모터 위상 제어},pdfauthor={이해리}}
\begin{document}
\renewcommand{\small}{\fontsize{8.7}{10.5}\selectfont}
\setlength{\leftmargini}{1.35em}

\begin{frame}{LAB3-22 · 스텝모터 위상 제어}
\small
전자전기컴퓨터설계실험Ⅱ · FPGA 응용 실습\par\vspace{0.28cm}
clock-enable마다 4상 코일 패턴을 이동하여 정·역회전과 정지를 제어한다.\par\vspace{0.28cm}
...
\small
clock-enable마다 4상 코일 패턴을 이동하여 정·역회전과 정지를 제어한다.\par\vspace{0.28cm}
핵심 개념: clock-enable, 입력 동기화, 4상 시퀀스, 방향 전환\par\vspace{0.28cm}
기능 TB: \texttt{tb\_stepper}\par\vspace{0.18cm}
예상 PASS: \texttt{LAB3\_STEPPER\_PASS checks=8}
\end{frame}

\begin{frame}{입출력과 보드 연결}
\small
clk\_50mhz B6 · rst\_p K4 · enable N8 · direction N4 · stepmotor[3:0] Y20/Y22/AA20/AA21\par\vspace{0.32cm}
...
\small
정방향 한 주기, 역방향 두 단계, enable=0 유지 조건을 가속된 step rate로 검사한다.\par\vspace{0.28cm}
...
git clone --branch v2.0.2 https://github.com/Glaysia/fpga-lab-template.git lab3_22_stepper
cd lab3_22_stepper
git switch -c main
...
경로 & 파일\\\hline
src/ & \texttt{lab3\_stepper.v}\\
sim/ & \texttt{tb\_stepper.sv}\\
constraints/ & \texttt{lab3\_stepper.xdc}\\
...
testbench는 \texttt{sim/tb\_stepper.sv}, simulation\_top은 \texttt{tb\_stepper}다.\par\vspace{0.25cm}
설계 top \texttt{lab3\_stepper}은 Vivado 합성에서 선택한다. TB module을 합성 top으로 선택하지 않는다.
\end{frame}
...
\lstinputlisting[language=,firstline=1,lastline=5,firstnumber=1]{../../../example/fpga_projects_hdl/LAB3/vivado_2026_1/04_stepper/simulation.json}
\end{frame}

\begin{frame}[fragile]{lab3\_stepper.v · 1–18행}
\lstinputlisting[language=Verilog,firstline=1,lastline=18,firstnumber=1]{../../../example/fpga_projects_hdl/LAB3/vivado_2026_1/04_stepper/src/lab3_stepper.v}
\end{frame}

\begin{frame}[fragile]{lab3\_stepper.v · 19–36행}
\lstinputlisting[language=Verilog,firstline=19,lastline=36,firstnumber=19]{../../../example/fpga_projects_hdl/LAB3/vivado_2026_1/04_stepper/src/lab3_stepper.v}
\end{frame}

\begin{frame}[fragile]{lab3\_stepper.v · 37–51행}
\lstinputlisting[language=Verilog,firstline=37,lastline=51,firstnumber=37]{../../../example/fpga_projects_hdl/LAB3/vivado_2026_1/04_stepper/src/lab3_stepper.v}
\end{frame}

\begin{frame}[fragile]{tb\_stepper.sv · 1–18행}
\lstinputlisting[language=Verilog,firstline=1,lastline=18,firstnumber=1]{../../../example/fpga_projects_hdl/LAB3/vivado_2026_1/04_stepper/sim/tb_stepper.sv}
\end{frame}

\begin{frame}[fragile]{tb\_stepper.sv · 19–23행}
\lstinputlisting[language=Verilog,firstline=19,lastline=23,firstnumber=19]{../../../example/fpga_projects_hdl/LAB3/vivado_2026_1/04_stepper/sim/tb_stepper.sv}
\end{frame}

\begin{frame}[fragile]{lab3\_stepper.xdc · 1–12행}
\lstinputlisting[language=,firstline=1,lastline=12,firstnumber=1]{../../../example/fpga_projects_hdl/LAB3/vivado_2026_1/04_stepper/constraints/lab3_stepper.xdc}
\end{frame}
...
\item \texttt{LAB3\_STEPPER\_PASS checks=8}와 종료 시각을 기록한다.
\item 03 Open waveform으로 \texttt{build/sim/wave.vcd}를 연다.
\end{enumerate}
\end{frame}

\begin{frame}{VS Code · 프로젝트와 파일}
\begin{center}\includegraphics[width=\textwidth,height=6.0cm,keepaspectratio]{captures/04_stepper-vscode-workspace.png}\end{center}
{\tiny 04\_stepper-vscode-workspace}
\end{frame}

\begin{frame}{VS Code · 시뮬레이션 PASS}
\begin{center}\includegraphics[width=\textwidth,height=6.0cm,keepaspectratio]{captures/04_stepper-vscode-pass.png}\end{center}
{\tiny 04\_stepper-vscode-pass}
\end{frame}

\begin{frame}{VS Code · 파형}
\begin{center}\includegraphics[width=\textwidth,height=6.0cm,keepaspectratio]{captures/04_stepper-vscode-wave.png}\end{center}
{\tiny 04\_stepper-vscode-wave}
\end{frame}
...
\begin{frame}{수정 실험}
\small
STEP\_HZ를 절반으로 바꾸어 단계 간격을 계산하고 회전 속도를 비교한 뒤 복구한다.\par\vspace{0.28cm}
...
\item Design Sources의 top을 \texttt{lab3\_stepper}으로 설정한다.
\end{enumerate}
\end{frame}

\begin{frame}{Vivado · 소스와 top}
\begin{center}\includegraphics[width=\textwidth,height=6.0cm,keepaspectratio]{captures/04_stepper-vivado-sources.png}\end{center}
{\tiny 04\_stepper-vivado-sources}
\end{frame}

\begin{frame}{Vivado XSim}
\small
Flow Navigator → Simulation → Run Behavioral Simulation.\par\vspace{0.28cm}
Tcl Console의 \texttt{LAB3\_STEPPER\_PASS checks=8}를 확인한다. Objects/Wave에서 입력과 출력을 추가하고 Zoom Fit으로 전체 실행을 본다.\par\vspace{0.28cm}
Icarus와 XSim은 같은 RTL·TB를 실행해야 한다. 서로 다른 사본을 등록하지 않는다.
\end{frame}

\begin{frame}{Vivado · XSim PASS와 파형}
\begin{center}\includegraphics[width=\textwidth,height=6.0cm,keepaspectratio]{captures/04_stepper-xsim-pass.png}\end{center}
{\tiny 04\_stepper-xsim-pass}
\end{frame}
...
\begin{center}\includegraphics[width=\textwidth,height=6.0cm,keepaspectratio]{captures/04_stepper-vivado-bit.png}\end{center}
{\tiny 04\_stepper-vivado-bit}
\end{frame}
...
드라이버 결선 후 회전 방향·단계 속도·정지 유지와 모터 발열을 관찰한다.\par\vspace{0.25cm}
