...
\setbeamertemplate{footline}{%
  \begin{tikzpicture}[remember picture,overlay]
    ...
    \node[anchor=west,rounded corners=2pt,fill=UOSBlue!55,text=white,font=\scriptsize] at ([xshift=0.65cm,yshift=0.52cm]current page.south west) {\hyperlink{vivado-install-contents}{목차}};
...
\newcommand{\presentationdate}{2026. 09. 10.}
...
\newcommand{\screen}[2]{%
\begin{center}
\includegraphics[width=\linewidth,height=5.0cm,keepaspectratio]{assets/vivado-install/#1}\\[-0.05cm]
{\tiny\color{gray}#2}
\end{center}}
...
\AtBeginEnvironment{frame}{\fontsize{9}{12}\selectfont}
\begin{document}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north west,inner sep=0pt,text=white,font=\bfseries\fontsize{21}{27}\selectfont,align=left]
...
\begin{frame}[label=vivado-install-contents]{설치 순서}
\begin{enumerate}
\item 설치 파일 다운로드
...
\begin{frame}{2. 로그인과 설치 방식}
\screen{08-install-login.png}{화면 B 01:10 · 계정 입력 전}
\end{frame}
\begin{frame}{2. 로그인과 설치 방식 · 누르는 순서}
...
\begin{frame}{3. 제품 선택}
\screen{09-select-vivado.png}{화면 B 01:49}
\end{frame}
\begin{frame}{3. 제품 선택 · 누르는 순서}
...
\begin{frame}{4. 도구와 드라이버}
\screen{10-install-options.png}{화면 B 01:55 · 디바이스는 다음 화면에서 확인}
\end{frame}
\begin{frame}{4. 도구와 드라이버 · 누르는 순서}
...
\begin{frame}{5. Spartan-7 선택}
\screen{11-device-selection.png}{화면 B 01:58 · 녹화에는 Virtex-7이 선택되어 있음}
\end{frame}
\begin{frame}{5. Spartan-7 선택 · 누르는 순서}
...
\begin{frame}{6. 약관 확인}
\screen{12-license-agreements.png}{화면 B 01:59 · 동의 전 화면}
\end{frame}
\begin{frame}{6. 약관 확인 · 누르는 순서}
...
\begin{frame}{7. 설치 경로와 오류 확인}
\screen{13-destination-errors.png}{화면 B 02:12 · 오류 때문에 Next가 비활성화된 예시}
\end{frame}
\begin{frame}{7. 설치 경로와 오류 확인 · 누르는 순서}
...
\begin{frame}{9. License Manager}
\screen{02-obtain-license.png}{화면 A 00:05}
\end{frame}
\begin{frame}{9. License Manager · 누르는 순서}
...
\begin{frame}{10. 무료 BASIC 선택}
\screen{03-basic-license.png}{화면 A 01:05}
\end{frame}
\begin{frame}{10. 무료 BASIC 선택 · 누르는 순서}
...
\begin{frame}{11. 사용할 PC 지정}
\screen{04-node-license.png}{화면 A 01:30 · Host ID 선택 전}
\end{frame}
\begin{frame}{11. 사용할 PC 지정 · 누르는 순서}
...
\begin{frame}{15. Vivado 실행}
\screen{06-vivado-basic.png}{화면 A 16:25}
\end{frame}
\begin{frame}{15. Vivado 실행 · 누르는 순서}
...
\begin{frame}{16. 확인용 프로젝트}
\screen{07-rtl-project.png}{화면 A 18:00}
\end{frame}
\begin{frame}{16. 확인용 프로젝트 · 누르는 순서}
...
PowerShell에서 실행한다.
\terminalbox{\$vivadoBin = 'C:/AMDDesignTools/2026.1/Vivado/bin'\\Test-Path "\$vivadoBin/xvlog.bat"\\Test-Path "\$vivadoBin/xelab.bat"\\Test-Path "\$vivadoBin/xsim.bat"}

\begin{itemize}
\item 세 결과가 모두 \texttt{True}인지 확인한다.
\item 다른 위치에 설치했다면 위 확인 명령의 경로를 맞춘다.
\item 파일 존재 확인 뒤 실제 시뮬레이션·VCD 생성은\\
별도 조합회로 시뮬레이션 매뉴얼에서 진행한다.
\end{itemize}
\end{frame}

\begin{frame}{19. 환경 변수 설정 열기}
\screen{14-system-properties.png}{화면 C · 2026.09.07 실제 Windows 설정 화면}
\end{frame}
\begin{frame}{19. 환경 변수 설정 열기 · 누르는 순서}
...
\terminalbox{where.exe xvlog.bat\\where.exe xelab.bat\\where.exe xsim.bat\\xsim.bat -version}
...
VS Code 사전 실습은 Icarus를 사용하며 Vivado 실행은 별도 GUI 절차를 따른다.
\end{frame}
...
\item \textbf{터미널이 Vivado 도구를 찾지 못함}\\
실제 설치 경로와 사용자 PATH를 비교한다.\\
