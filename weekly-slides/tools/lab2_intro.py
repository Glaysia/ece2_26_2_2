...
def main():
    from lab2_series import template_guidance
    ...
    frame('같은 템플릿, 다른 폴더 이름',paras('Windows PowerShell에서 한 줄씩 실행한다. 줄 끝의 `는 이어 쓰기다.')+terminal('git clone --branch v2.0.1 `','  https://github.com/Glaysia/fpga-lab-template.git `','  lab2_01_counter','cd lab2_01_counter','git switch -c main','code LAB1.code-workspace')+paras('다음 실험은 마지막 폴더 이름만 바꾼다. LAB1.code-workspace는 공통 템플릿 파일명이므로 그대로 연다.'))
    ...
    frame('v2.0.1 도구 설정 확인',template_guidance())
