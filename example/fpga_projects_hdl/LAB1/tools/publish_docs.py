...
def collect(p):
    ...
    gui_record=evidence/'gui-simulation.json'
    if gui_record.exists():
        gui=json.loads(gui_record.read_text(encoding='utf-8'))
        stages['gui']={'status':gui['status'],'cases':gui['cases'],'end_ns':gui['end_ns'],'evidence':'gui-simulation.json','comparison':gui['standalone_comparison']['status'],'bit_generation':'Existing batch bit inspected in GUI; not regenerated by this GUI run'}
