...
1. **추가 GUI 캡처:** Windows 잠금 화면이 실제 관찰되어 중단했다. 로그인한 데스크톱에서 최신 06–10과 두 통합본의 최종 RTL·TB, 회로별 파형·Vivado 결과 화면을 확보한다. 공통 화면과 이미 확보한 최신 01–05 소스 이미지는 그 범위만 증명한다. 레거시의 새 VS Code 안내도 최종 배포 파일과 대조한다.
2. **캡처 반영·가독성:** 각 코드 화면을 의미 단위로 나누고 파일명·해설을 연결한다. 업데이트한 TeX를 2회 빌드하고 실제 PDF의 이미지 크기·잘림·목차를 확인한 후 ZIP을 갱신한다. 원본 PNG는 새로 추적하지 않는다.
