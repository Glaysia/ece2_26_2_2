...
## 회로별 수정 과제와 실제 문법 오류 복구

22개 매뉴얼에는 각 회로의 RTL 한 곳을 바꾸고 동일한 자기검사 TB로 실패를 확인한 뒤 복구하는 과제가 있다. 최신 개별 10개와 통합 1개에서 정상·변경·복구 총 33회 실행을 확인했다. 입력 파일 해시와 원본 로그는 [수정 실험 검증](student-modification-validation.json)에 있다. 레거시의 사전 실습은 대응 최신판과 동일하며, 두 통합본의 RTL 14개·TB는 바이트 단위로 동일하다.

첫 회로에서는 실제 VS Code 창에서 세미콜론 누락 → 작업 실패 → Problems 두 번 클릭으로 오류 행 이동 → 세미콜론 복구·저장 → 작업 재실행을 수행했다. 4개 PASS와 40000ps 종료를 확인했다. [GUI 실행 기록](student-syntax-gui-validation.json)의 입력 해시는 기존 정상 학생 실행과 동일하다. 실제 화면은 첫 매뉴얼의 문법 오류 복구 두 페이지에 수록한다.

이 검증은 회로 수정과 도구 조작에 대한 소프트웨어 증거다. 실제 보드 기록·사진·영상의 미수행 상태는 바뀌지 않았다.
