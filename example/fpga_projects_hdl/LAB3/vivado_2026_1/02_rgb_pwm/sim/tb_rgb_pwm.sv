`timescale 1ns/1ps
module tb_rgb_pwm;
    reg clk=0,rst=1,button_r=0,button_g=0,button_b=0;
    wire [3:0] led_r,led_g,led_b;
    integer checks=0,i,hr,hg,hb;
    always #5 clk=~clk;
    lab3_rgb_pwm #(.CLK_HZ(1000),.PWM_HZ(100),.LEVELS(10),.DEBOUNCE_CYCLES(2)) dut
      (.clk(clk),.rst(rst),.button_r(button_r),.button_g(button_g),.button_b(button_b),
       .led_r(led_r),.led_g(led_g),.led_b(led_b));

    task press_r; begin button_r=1;repeat(6)@(posedge clk);button_r=0;repeat(6)@(posedge clk);end endtask
    task press_g; begin button_g=1;repeat(6)@(posedge clk);button_g=0;repeat(6)@(posedge clk);end endtask
    task press_b; begin button_b=1;repeat(6)@(posedge clk);button_b=0;repeat(6)@(posedge clk);end endtask
    task measure(input integer er,input integer eg,input integer eb);
    begin
      wait(dut.u_pwm_r.count==0);hr=0;hg=0;hb=0;
      for(i=0;i<10;i=i+1)begin @(posedge clk);#1;hr+=led_r[0];hg+=led_g[0];hb+=led_b[0];end
