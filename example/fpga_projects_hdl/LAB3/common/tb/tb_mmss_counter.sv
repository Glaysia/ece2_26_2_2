...
module tb_mmss_counter;
    ...
    integer checks=0;
    always #5 clk=~clk;
    mmss_counter #(.CLK_HZ(2)) dut(.clk(clk),.rst(rst),.minute_tens(mt),.minute_ones(mo),.second_tens(st),.second_ones(so));
    sevenseg_decode decoder(.digit(decode_digit),.segments(decoded_segments));
    task advance(input integer seconds);begin repeat(seconds*2)@(posedge clk);#1;end endtask
    task check_time(input integer emt,input integer emo,input integer est,input integer eso);
    begin if(mt!==emt||mo!==emo||st!==est||so!==eso)$fatal(1,"time=%0d%0d:%0d%0d",mt,mo,st,so);checks++;end endtask
    initial begin
      ...
      repeat(2)@(posedge clk);rst=0;check_time(0,0,0,0);
