...
module tb_uart_echo;
    ...
    lab3_uart_echo #(.CLK_HZ(800),.BAUD(100)) dut
      (.clk(clk),.rst(rst),.uart_rxd(uart_rxd),.uart_txd(uart_txd),.led(led));
    task send_byte(input [7:0] value);
    begin
      uart_rxd=0;repeat(DIV)@(posedge clk);
      for(i=0;i<8;i=i+1)begin uart_rxd=value[i];repeat(DIV)@(posedge clk);end
      uart_rxd=1;repeat(DIV)@(posedge clk);
    end endtask
    task receive_byte(input [7:0] expected);
    begin
      observed=0;@(negedge uart_txd);repeat(DIV+DIV/2)@(posedge clk);
      for(i=0;i<8;i=i+1)begin observed[i]=uart_txd;repeat(DIV)@(posedge clk);end
      if(observed!==expected)$fatal(1,"echo=%h expected=%h",observed,expected);
      if(led!==expected)$fatal(1,"led=%h expected=%h",led,expected);
      checks=checks+1;repeat(DIV)@(posedge clk);
    end endtask
    initial begin
