...
> 2026-09-13 범위 변경: 현재 제작 기준은 [LAB2 매뉴얼 10개 완성 계획](GOAL0913_LAB2.md)이다. 최신 Vivado 8개·통합 Vivado 1개·통합 CLI 1개를 제작한다. 아래의 구버전 포함 18개 범위와 GUI 중단 문구는 과거 이력이며 현재 지침으로 적용하지 않는다.
...
- 최신 TeX 20개(개별 18개·공통 예습·전체 목차), 총 1,146쪽을 모두 2회 빌드했다. 영역 넘침·글자 누락 경고는 0개다.
- [초안 구조 검사](../weekly-slides/weekly-slides/LAB2_FPGA_0921/draft-structure-validation.json)에 PDF/TeX 해시를 기록했다. 모든 페이지의 4:3 비율과 2쪽 목차 복귀, 내부 목적지, 상대 PDF 파일 및 목적지, 원고별 페이지 수와 누락 표시 수를 확인했다. 전체 페이지 시각 검수는 아니다.
- CLI판 장치 확인 페이지에 섞인 Vivado 메뉴를 제거하고 openFPGALoader 탐지·SRAM 기록·8모드 관찰 순서로 고쳤다. 수정 페이지를 렌더링해 잘림 여부를 확인했다.
- 패키징은 캡처 이미지·메타데이터·현재 소스 해시를 다시 검사한다. 현재 97개 실제 이미지가 없어 300개 사용 구간이 미완료이며, 실제 실행에서 ZIP 생성 거부를 확인했다. GUI 조작 중단을 유지한다.
